...
public class CoreJewishCalendar: JewishDateWrapper, JewishDateRepresentable, @unchecked Sendable {
    ...
    public required convenience init(withJewishYear year: Int, andMonth month: JewishMonth, andDay day: Int, isInIsrael: Bool = false) {
    ...
    public func copy(year: Int? = nil, month: JewishMonth? = nil, day: Int? = nil, isInIsrael: Bool? = nil) -> Self {
		Self.init(withJewishYear: year ?? self.year, andMonth: (month ?? self.month), andDay: day ?? self.day, isInIsrael: isInIsrael ?? self.isInIsrael)
    }
    
    public func copy(date: Date, includeTime: Bool = false, isInIsrael: Bool? = nil) -> Self{
		Self.init(date: date, includeTime: includeTime, isInIsrael: isInIsrael ?? self.isInIsrael)
    }
	
	/// Create a copy of the ``CoreJewishCalendar`` instance advanced by a given calendar component
	/// - Parameters:
	...
	/// - Warning: This method will crash in rare situations where `Calendar's` `Calendar.date(byAdding:value:to:wrappingComponents:)` returns `nil`.
	/// ## See Also
    ...
    public func getUpcomingParsha() -> Parsha {
        var newCalendar: CoreJewishCalendar
    ...
    public func getWeeklyParsha() -> Parsha {
        ...
        var cal = CoreJewishCalendar(date: nextShabbos, isInIsrael: isInIsrael)
        
        while cal.getParsha() == .none {
            nextShabbos = nextShabbos.next(.saturday)
            cal = CoreJewishCalendar(date: nextShabbos, isInIsrael: isInIsrael)
        }
	...
	static var chagCheckers: [JewishHoliday: (CoreJewishCalendar) -> Bool] {
		[
    ...
    public var tomorrow: Self {
		Self.init(date: gregDate.withAdded(days: 1)!, isInIsrael: isInIsrael)
    }
    ///Return the ``CoreJewishCalendar`` for the previous day
    public var yesterday: Self {
		Self.init(date: gregDate.withAdded(days: -1)!, isInIsrael: isInIsrael)
    }
    
    public var chagStart: CoreJewishCalendar {
		var temp = Self.init(date: gregDate, isInIsrael: isInIsrael)
    ...
    public var chagHavdallahDate: CoreJewishCalendar {
